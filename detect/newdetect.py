...
class IMAGE_LEFT:
    def __init__(self, image=None, point:list=[]):
        print("IMAGE_LEFT_INIT")
    ...
    def image_callback(self, msg):  # feedback means actual value.
        ...
        if len(contours) > 0:
            ...
            self.point.append(xmin)
            self.point.append(xmax)
            self.point.append(ymin)
            self.point.append(ymax)
...
class IMAGE_RIGHT:
    def __init__(self, image=None, point:list = []):
        print("IMAGE_RIGHT_INIT")
        self.sub = rospy.Subscriber('/airsim_node/drone_1/front_right/Scene', Image, self.image_callback, queue_size=10)
        self.sub = rospy.Subscriber('/swap', String, self.String_callback, queue_size=10)
        self.image = image
        self.point = point
        self.target_low, self.target_up = np.array([5, 230, 100]), np.array([20, 255, 255])
...
def image_process(camera_left, camera_right, f, B, pexl):
    pose_pub = rospy.Publisher('/target', Point32, queue_size=10)
    left_positions = camera_left.point
    right_positions = camera_right.point
    image_x = 640
    image_y = 480
